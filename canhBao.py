...
def canhbao():
    ...
    cur.execute("""
        ...
        cb.noi_dung, cb.ngay_phat_hien, cb.trang_thai, tk.ten_tai_khoan
        FROM canh_bao cb 
        LEFT JOIN dulieuhinhanh dl ON cb.ma_hinh_anh = dl.ma_hinh_anh
        LEFT JOIN taikhoan tk ON cb.ma_tai_khoan = tk.ma_tai_khoan;
    """)
    data = cur.fetchall()
...
def update_status():
    ...
    ma_tai_khoan = session.get('ma_tai_khoan')  # Lấy mã tài khoản từ session
    ...
    cursor.execute("""
        UPDATE canh_bao 
        SET trang_thai='Đã xử lý', ma_tai_khoan=%s
        WHERE ma_canh_bao=%s
    """, (ma_tai_khoan, ma_canh_bao))
    conn.commit()

    # Lấy thông tin tài khoản vừa cập nhật
    cursor.execute("""
        SELECT tk.ten_tai_khoan 
        FROM canh_bao cb
        LEFT JOIN taikhoan tk ON cb.ma_tai_khoan = tk.ma_tai_khoan
    ...
    return jsonify({
        ...
        "tai_khoan": user_info["ten_tai_khoan"] if user_info else "Không xác định"
    })
...
def gui_email():
    sender_email = "bphuonggV.it@gmail.com"
    sender_password = "tcxyyqjzciucleud"
